...
'setPrefWidthPolicy' @ [25:12] ==> private fun <S> TreeTableColumn<(S..S?), *>.setPrefWidthPolicy(widthWithoutPaddingAndBorder: ObservableValue<Number>, getColumnWidth: (tableWidthWithoutPaddingAndBorder: Number, column: TreeTableColumn<(S..S?), *>) -> Number): Unit defined in net.corda.explorer.ui[SimpleFunctionDescriptorImpl]
Inferred types:
    <S> -> (S..S?)
...
'if (selectionModel.selectedItems.size == 0) {
            ...
            }' @ [53:13] ==> public final fun <`<TYPE-PARAMETER-FOR-IF-RESOLVE>`> `<SPECIAL-FUNCTION-FOR-IF-RESOLVE>`(thenBranch: SingleRowSelection<S>, elseBranch: SingleRowSelection<S>): SingleRowSelection<S>[SimpleFunctionDescriptorImpl]
Inferred types:
